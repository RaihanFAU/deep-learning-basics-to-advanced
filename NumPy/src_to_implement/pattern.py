...
class Checker:
    # initialize image resolution, tile size, and output container
    ...
    def draw(self):
        # create one black and one white square tile
        ...
        top_row = np.concatenate((black, white), axis = 1)
        bottom_row = np.concatenate((white, black), axis=1)
        tile = np.concatenate((top_row, bottom_row), axis=0)

        # repeat base block to fill the full image

        tiles = self.resolution// (2* self.tile_size)
        self.output = np.tile(tile, (tiles, tiles))
...
class Circle:
    # initialize circle resolution, radius, and center position
...
class Spectrum:
    # initialize image resolution and RGB container
